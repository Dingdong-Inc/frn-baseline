...
class Exp_Main(Exp_Basic):
    ...
    def predict(self, setting, load=False):
        ...
        df_res = self.cal_metrics(merged_df, groups=["psd>=0"],
                                  target_dates=target_dates)
    ...
    def data_organization(self, pred_data_path, target_dates):
        ...
        df_train = df_train[df_train.dt >= start_date]
        df_train_psd = df_train.groupby(group_ids)['sale_amount'].mean().to_frame('psd')
        ...
        unique_groups = df_eval[['store_id', 'product_id']].drop_duplicates().reset_index(drop=True)
        # 创建日期数据框
        date_df = pd.DataFrame({'date': target_dates})
        # 使用 merge 进行笛卡尔积
        expanded_df = unique_groups.assign(key=1).merge(date_df.assign(key=1), on='key').drop('key', axis=1)
        ...
        for group in groups:
            ...
            for target_date in target_dates:
                if len(target_dates) > 1:
                    df2 = df1[(df1.date == target_date) & (df1.stock_hour6_22_cnt == 0)]
                else:
