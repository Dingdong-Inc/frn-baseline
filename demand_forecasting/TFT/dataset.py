...
class Dataset:
    ...
    def gen_predict_df(self, df):
        ...
        decoder_data = df[df[self.config.dataset_config["target"][0]].isnull()]
    ...
    def gen_dataset(self):
        self.train_df["time_idx_max"] = self.train_df.groupby(self.config.dataset_config["group_ids"])['time_idx'].transform('max')

        if not isinstance(self.config.dataset_config['target'], list):
            target_normalizer = GroupNormalizer(groups=self.config.dataset_config['group_ids'],
                                                transformation="softplus")
        else:
            target_normalizer = MultiNormalizer(normalizers=[
                GroupNormalizer(groups=self.config.dataset_config['group_ids'], transformation="softplus")
                    for _ in self.config.dataset_config['target']
            ])

        categorical_encoders = [NaNLabelEncoder(add_nan=True)] * len(self.config.field_config['need_encode_na'])
        categorical_encoders = dict(zip(self.config.field_config['need_encode_na'], categorical_encoders))
        if self.config.valid:
            dataset = TimeSeriesDataSet(
                self.train_df[lambda x: x.time_idx <= x.time_idx_max - self.config.dataset_config['max_prediction_length']],
