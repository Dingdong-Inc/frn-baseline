...
LENGTH = 284
KERNEL_BANK = [
    # Keep only kernels with a weekly period.
...
def generate_time_series(max_kernels: int = 5):
    ...
    while True:
        ...
        # Sample a time series from the GP prior (cholesky with jitter, no retries needed)
        ts = sample_from_gp_prior_efficient(kernel=kernel, X=X, method="cholesky")
